...
## injuries ----
injury_counts <- trauma_2021_2025 |>
  dplyr::filter(
    grepl(pattern = "^ia$|iowa", x = Injury_State, ignore.case = T),
    !is.na(Injury_County),
    !Injury_County %in%
...
iowa_injury_counts_age <- trauma_2021_2025 |>
  dplyr::filter(
    grepl(pattern = "^ia$|iowa", x = Injury_State, ignore.case = T),
    !is.na(Injury_County),
    !Injury_County %in%
...
iowa_case_counts_age <- trauma_2021_2025 |>
...
trauma_data_na <- trauma_2021_2025 |>
...
export_state_data(
  x = injury_case_rates,
  output_dir_root = paste0(state_output_folder, "rates"),
  subfolder = ""
)
